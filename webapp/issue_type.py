...
class IssueTypeRepository:
    ...
    def get_paged_records(self,page_number):
        ...
        db = get_db()
        records = db.execute("""
SELECT id, title FROM issue_type 
ORDER BY title ASC
LIMIT ? OFFSET ?;
""", (self.page_size, offset)).fetchall()
    ...
    def get_record(self, id):
        db = get_db()
        record = db.execute("""
SELECT id, title FROM issue_type WHERE id = ?;
""", (id,)).fetchone()
        return record

    def add_new_record(self, issue_type_title):
        db = get_db()
        db.execute('INSERT INTO issue_type (title) VALUES (?);',
            (issue_type_title,)
        )
        db.commit()

    def update_record(self, id, issue_type_title):
        db = get_db()
        db.execute('UPDATE issue_type SET title = ? WHERE id = ?;',
            (issue_type_title, id)
        )
        db.commit()
...
def register():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            issue_type_repository.add_new_record(issue_type_title)
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            if action == 'Delete':
                ...
                issue_type_repository.update_record(id, issue_type_title)
            return redirect(url_for('issue-type.index'))
